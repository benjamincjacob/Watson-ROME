...
var searchDiscovery = Promise.promisify(erp.searchDiscovery.bind(erp));
					...
					if (responseContext.hasOwnProperty('api')) {
						...
						switch (responseContext.api.RUN) {
							...
							case 'SearchDiscovery':
								debug('Calling Watson Discovery');
								
								return searchDiscovery(input.text)
									.then(function(SearchResult) {
										extend(responseContext.api, SearchResult);
										delete responseContext.api.RUN;
										message = {
											input: messageResponse.input,
											context: responseContext
