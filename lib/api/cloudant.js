...
var botdb = cloudant.db.use('salesbotdb');
var chatlog = cloudant.db.use('saleschatlog');
...
module.exports = {
  ...
  getlog: function(params, callback) {
    chatlog.get(params, function(err, response) {
      if (err) {
        if (err.error !== 'not_found') {
          return callback(err);
        } else {
   ...
   * @return {void}
   */
  putlog: function(params, callback) {
    
    var id = uuid.v4();
    params._id = id;
    params.time = new Date();
    chatlog.insert(params, callback);
